...
st = client.get_waveforms("GS", "CA03", "*",
...
t=st[0].times()
ax1.plot(t, st[0].data, label=st[0].id, color='#377eb8', alpha=.7)
ax1.tick_params(axis='y', colors='#377eb8')
ax1.text(-12, 120000, '(a)')
ax2 = ax1.twinx()
t = st[1].times()
...
ax1.plot(t, 1000*st[0].data/sen, label=st[0].id, color='#377eb8', alpha=.7)
ax1.tick_params(axis='y', colors='#377eb8')
ax1.text(-12, 0.25, '(b)')
ax2 = ax1.twinx()
t = st[1].times()
...
t=st[0].times()
ax1.plot(t, st[0].data*1000, label=st[0].id, color='#377eb8', alpha=.7)
ax1.text(-12, 9.5, '(c)')
t = st[1].times()
ax1.plot(t,st[1].data*1000, label=st[1].id, color='#ff7f00', alpha=.5)
ax1.set_xticklabels([])
ax1.set_yticks([-7, 0, 7])
